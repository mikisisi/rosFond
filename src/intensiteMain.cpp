...
int main(int argc, char *argv[])
{
    ...
    char buffer[256];
    int valeurFin=205;
    float a2dVal[4] = {510,5110,510,0}; // destine a contenir les valeurs des intensites
...
float bufCourant[65];
 ADCmcp3008Spi convertisseurIntensites("/dev/spidev0.0", SPI_MODE_0, 1000000, 8);
    ...
    std_msgs::Float32MultiArray tableauAEnvoyer ;

    ros::init(argc,argv,"intensite_node");

    ros::NodeHandle noeud;

    ros::Publisher publisherIntensite = noeud.advertise<std_msgs::Float32MultiArray>("intensite",1000);
...
for(a2dChannel=0; a2dChannel<4; a2dChannel++)
	   ...
	   data[0] = 1;  //  first byte transmitted -> start bit
         ...
         bufCourant[i]=a2dBuff[a2dChannel] ;
              ...
              intensity/=65;
                  ...
                  intensity=intensity *6*66*1.33/10;

     a2dVal[a2dChannel]=absoluteValueFloat(intensity) ;
	}
	...
	std::cout << a2dVal[a2dChannel]<<"\t" ; 

            tableauAEnvoyer.data.push_back(a2dVal[a2dChannel]);
        std::cout << tableauAEnvoyer.data[a2dChannel]-a2dVal[a2dChannel] << std::endl ;
}
std::cout << endl;
        publisherIntensite.publish(tableauAEnvoyer);
